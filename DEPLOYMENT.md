...
**Backend Self-Registration (V6, BSQ-22) — verify this grant against your actually-deployed role.**
`V6__backend_self_registration.sql` adds `backends.announced_by`, and self-registration (`POST
/backends/announce`) is the Gateway application role's **first `INSERT` into `backends`** — previously
this role only ever `UPDATE`d that table (via the backend-health checker). The table-level `INSERT` grant
above already covers `announced_by` automatically (it's a plain column, not a separate grant target), and
no sequence grant is needed beyond what's already listed (`backends.id` is `GENERATED BY DEFAULT AS
IDENTITY`) — but do not assume this from the grant block above alone: roles drift over time, and this
project's own architecture doc originally claimed "`INSERT` was never granted" for this role, which turned
out to be false once actually checked against this file. Re-run `\dp backends` (or equivalent) against
your deployed role before enabling `BACKEND_SELF_REGISTRATION_ENABLED` in production. The grant above is
also wider than this feature strictly needs — `DELETE` on `backends` is granted but no code path in this
feature (or anywhere else in the Gateway) ever deletes a backend row (`DELETE /backends/{name}` is a soft
decommission, `status = OFFLINE`, never a row delete — `review_jobs`/`review_results` FK `backends(id)`
with no `ON DELETE`). Revoking it (`REVOKE DELETE ON backends FROM review_gateway;`) is a recommended,
optional hardening step this migration does not apply automatically (Flyway does not manage
GRANT/REVOKE in this project).
...
**Backend Self-Registration (V6, off by default): there are now three ways to register a backend.**
`AdminController` exposes `POST /backends` (ADMIN) and `DELETE /backends/{name}` (ADMIN) in addition to
`GET /backends`/`GET /metrics`, and `BackendAnnounceController` exposes `POST /backends/announce`
(WORKER role, only when `gateway.backend.self-registration.enabled=true`) — a raw SQL insert is no longer
the only way. Pick the path that matches your topology:

1. **Self-registration (recommended for the reference 1:1 Worker-per-`llama-server` topology).** Set
   `BACKEND_SELF_REGISTRATION_ENABLED=true` and `BACKEND_ALLOWED_HOST_PATTERN` (Gateway), then set
   `BACKEND_URL` on each Worker ([§6.1](#61-environment-file)) — the Worker announces itself once, before
   it starts polling for jobs, and this step (registering rows by hand) is skipped entirely. See root
   [README §4.6](README.md#46-backend-self-registration-optional) for the full design and the
   `BACKEND_ALLOWED_HOST_PATTERN` startup gate this depends on.
2. **Admin API** (`POST /backends`, `ADMIN` token) — for registering/updating a backend without touching
   the Worker's own config, or for setting fields self-registration can never set (`capacity`, `status`,
   `structuredOutputMode`, `promptMessageFormat`):

   ```bash
   curl -s -X POST http://gateway.internal/backends \
     -H "Authorization: Bearer $ADMIN_TOKEN" -H "Content-Type: application/json" \
     -d '{"name":"llama-01","url":"http://192.168.1.101:8000","model":"qwen2.5-coder","capacity":1}'
   ```
3. **Raw SQL** (break-glass / scripted bulk-seed only, e.g. `docker-compose.yml`'s `backend-seed`
   one-shot job) — a direct insert into the `backends` table created in [§3](#3-step-1-postgresql) still
   works and is unaffected by this feature:
...
Whichever path you use, the `name` you choose here (`llama-01`) is exactly the string the Worker must be
configured with as `BACKEND_ID` in [§6](#6-step-4-deploy-the-worker) — despite the field being named
`backendId` on the wire (`POST /jobs/claim`), it carries this **name**, not the numeric `id` column. For
path 1, `BACKEND_ID` and `BACKEND_URL`/`name` are set on the Worker side only — there is no separate
"register" step to run against the Gateway.
...
| `BACKEND_ALLOWED_HOST_PATTERN` | `gateway.backend.allowed-host-pattern` | нет (`.*`), но **обязательна фактически**, если `BACKEND_SELF_REGISTRATION_ENABLED=true` | Сузить до реальной сети `llama-server` (защита от SSRF). Backend Self-Registration (V6): когда self-registration включён, Gateway **отказывается стартовать**, если этот паттерн покрывает целиком хотя бы одну из тестовых "форм" хостов (dotted-имена / IP-литералы / однословные имена) — см. §8d ниже, подраздел Backend Self-Registration |
| `BACKEND_SELF_REGISTRATION_ENABLED` | `gateway.backend.self-registration.enabled` | нет (`false`) | Kill-switch для `POST /backends/announce` (роль `WORKER`) — см. §8d ниже |
| `BACKEND_MAX_BACKENDS` | `gateway.backend.self-registration.max-backends` | нет (`16`) | Жёсткий предел числа строк `backends`, которые `announce` может ВСТАВИТЬ (не ограничивает `UPDATE` существующей строки и admin-путь `POST /backends`) — см. §8d ниже |
...
**Gateway, §A — Backend Self-Registration (опционально, выключено по умолчанию, BSQ-01..24):**

Позволяет Worker'у самому зарегистрироваться в реестре `backends` при старте (`POST
/backends/announce`, роль `WORKER`) вместо (или в дополнение к) ручного `INSERT`/admin API — см. root
[README §4.6](README.md#46-backend-self-registration-optional) для полного описания и
`docs/backend-self-registration-threat-model.md` для модели угроз (`BSQ-01`..`BSQ-24`).

- **`BACKEND_SELF_REGISTRATION_ENABLED=true` требует `BACKEND_ALLOWED_HOST_PATTERN` уже суженным.**
  Это проверяется на старте (`GatewayProperties.validateBackendSelfRegistrationOnStartup`) через набор
  случайных и фиксированных sentinel-хостов трёх "форм" (dotted-имена, IP-литералы, однословные имена):
  если сконфигурированный паттерн матчит **все** sentinel-хосты хотя бы одной формы целиком, старт
  отказывается с сообщением, прямо говорящим: **прохождение этой проверки доказывает только то, что
  паттерн не универсален хотя бы для одной проверенной формы — никогда не то, что он достаточно узок.**
  Не путайте "стартовало" с "паттерн безопасен" — это ответственность оператора, проверка — лишь
  fail-fast от самой грубой ошибки (`.*`, `.+`, `[\s\S]*` и т.п.).
- **`BACKEND_MAX_BACKENDS` (default `16`)** ограничивает только INSERT-ветку `announce` (BSQ-03) — не
  ограничивает обновление уже существующей строки тем же `workerId`, и не ограничивает admin-путь
  (`POST /backends`, всегда доверенный принципал). При достижении предела Gateway отвечает `503
  BACKEND_REGISTRY_FULL` (не `422`) — это транзиентное состояние, а не постоянная ошибка конфигурации
  Worker'а; Worker ретраит с backoff, а не падает насовсем.
- **`http://` backend-URL принимается сознательно, это НЕ упущение.** В отличие от `GITLAB_BASE_URL`
  (который обязан быть `https://`, SR-15), URL backend'а (`llama-server`) может быть `http://` — эти
  хосты по конструкции находятся в приватной LAN-сети оператора (Mac mini и т.п.), а не в публичном
  интернете, и SSRF-защита (`allowed-host-pattern` + блокировка loopback/link-local/metadata-адресов)
  действует независимо от схемы. Это задокументированное отклонение от общего принципа "только HTTPS",
  а не недосмотр — см. root [README §4.3](README.md#43-deployment-must-dos-from-docssecurityfeature-03-sast-reportmd).
- **`MAINTENANCE`/`OFFLINE` НЕ блокирует URL от перезаписи.** Перевод backend'а в `MAINTENANCE`/`OFFLINE`
  (через `POST /backends` или напрямую в БД) отключает его от диспетчеризации новых job'ов, но **не**
  "запирает" его `url` — следующий успешный `announce` от любого Worker'а с правильным `workerId` всё
  равно обновит `url` этой строки (Gateway при этом громко пишет `WARN`, но объявление проходит успешно).
  Если нужно гарантированно защитить URL строки от перезаписи, единственный способ — сменить `name`
  строки (тогда объявления под старым именем создают/обновляют отдельную запись) либо остановить Worker,
  который может объявиться под этим именем.
- **Аудит — только в логах, не в PostgreSQL.** Каждая мутация `backends` через self-registration
  (регистрация новой строки, смена владения, смена `url`) пишет структурированную `INFO`/`WARN`-строку в
  лог Gateway'я, но **не** создаёт строку в `review_events` (эта таблица требует `review_id`, которого у
  мутации реестра backend'ов нет). `backends.updated_at` перезаписывается следующим же успешным
  health-probe (в пределах `gateway.scheduler.backend-health-interval`, по умолчанию 60с), так что и он
  не годится как журнал. **Практическое следствие: retention логов Gateway — это и есть retention аудита
  для этой фичи.** Если требуется форензика дольше, чем хранятся логи, настройте retention логов
  соответствующим образом (шифрование/backup логов — уже упомянуты в
  [README §4.3](README.md#43-deployment-must-dos-from-docssecurityfeature-03-sast-reportmd) п.5 для
  других данных; то же соображение применимо и здесь).
...
| `BACKEND_URL` | `backend.url` | нет (не задан — self-registration выключен) | Backend Self-Registration: **опционально, отдельного флага-включателя нет — само наличие значения и есть переключатель**. Адрес `scheme://host[:port]` (bare origin, без пути/query/userinfo), по которому **Gateway** будет health-probe'ить этот backend — НЕ то же самое, что `LLAMA_URL` ниже (куда подключается сам процесс Worker'а, по умолчанию loopback; спутать их — самая частая ошибка оператора здесь). При задании `WorkerProperties.validateBackendUrl()` прерывает старт, если значение не парсится как URI, не `http(s)://`, резолвится в loopback-хост, или не bare origin — та же проверка, что Gateway делает на своей стороне (`422 BACKEND_URL_REJECTED`), только с точным сообщением о причине вместо непрозрачного кода ошибки. Требует со стороны Gateway `BACKEND_SELF_REGISTRATION_ENABLED=true` — иначе Worker получает `403`/`404` (не фатально, WARN, переход в legacy-режим — регистрация через admin API/SQL, см. §5 выше) |
...
Break-glass / bulk-seed only — see [§5](#5-step-3-register-the-llama-backend) for the two preferred paths
(self-registration, admin `POST /backends`) that ship as of V6:
...
# 3. Register a backend (§5 -- shown here as direct SQL for a same-host smoke test; production
#    deployments should prefer self-registration or POST /backends, see §5)
