...
| `gateway.structured.max-findings-per-file` | — | `20` | Schema's `maxItems` for one file's `findings` array; re-checked on the received response too (never trusts the decoder constraint alone). |
| `gateway.structured.max-comment-chars` / `max-suggestion-chars` | — | `1200` / `2000` | Per-finding `maxLength` bounds, likewise both schema-enforced (when constrained) and re-checked on receipt. |
